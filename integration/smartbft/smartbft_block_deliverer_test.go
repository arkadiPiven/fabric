...
func smartBftBlockDelivererTest(
	...
	go func() {
		...
		for {
			...
			env := ordererclient.CreateBroadcastEndorserTransactionEnvelope(network, network.Orderers[1], channel, buff)
	...
	/* Start the mocks */
	var mocksArray [4]*MockOrderer
	resultChan := make(chan string)
	/* Probably need to remember the mocks here */
	...
	for i, orderer := range network.Orderers {
		...
		mo, err := NewMockOrderer(network.OrdererAddress(orderer, nwo.ListenPort), ledgerArray, secOpts, resultChan)
		Expect(err).NotTo(HaveOccurred())
		mo.logger.Infof("Mock orderer started at port %v", network.OrdererAddress(orderer, nwo.ListenPort))
		mocksArray[i] = mo
	}
	...
	//_, err = network.PeerAdminSession(peer, commands.ChannelJoin{
	...
	/* Create a peer and join to channel */
	...
	result := <-resultChan
	fmt.Println(result)
	fmt.Println("peer joined channel")
	//
